...
function MyTextField({ isInvalid, ...props }) {
  const invalid = isInvalid(props.value);

  return <TextField {...props} error={Boolean(invalid)} helperText={invalid || props.helperText} />;
}
...
function ValidationAndErrorDisplay() {
  ...
  const [inputs, setInputs] = useState([
    ...
    },
    {
      ...
      isInvalid: value => (value === '' || /\S+@\S+\.\S+/.test(value) ? false : 'Woops. Not a valid email address'),
    },
  ]);
